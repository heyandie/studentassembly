      ...
      .form__wrapper
        form(action="/api/report" method="post" enctype="multipart/form-data")
          ...
          .form__element
            ...
            textarea(rows="10" name="text" placeholder="State the corruption case in detail." v-model="request.report.text")
          .form__element
            .form__label Attachments
            .form__note It can be a document or an image. Each file has a 3MB limit.
            .form__attachments
              .form__attachment
                input(type="file" id="file1" accept="image/*" v-model="request.report.files[0]")
                label(for="file1") {{ file1 }}
              .form__attachment
                input(type="file" id="file2" accept="image/*" v-model="request.report.files[1]")
                label(for="file2") {{ file2 }}
              .form__attachment
                input(type="file" id="file3" accept="image/*" v-model="request.report.files[2]")
